...
try {
  ...
  const typedConsumer = [
    ...
    `import type { NikaCompileRemoteAuthoring, NikaCompileRemoteLimits } from '${packageName}';`,
    ...
    "const authoring: NikaCompileAuthoringOptions = { model: 'provider/model', strategy: 'only', repairs: 2, samples: 2, maxTokens: 8192, timeoutSeconds: 120, knowledge };",
    "void client.compile('intent', { decisionModel: 'typesafe/jev-1.13.0', authoring });",
    '// @ts-expect-error samples still require an explicit authoring model',
    "void client.compile('intent', { authoring: { samples: 2 } });",
    '// @ts-expect-error decision selection is a model string, never raw argv',
    "void client.compile('intent', { decisionModel: ['--raw'] });",
    "void client.schedule('flow.nika', { id: 'daily', when: { kind: 'once', at: '2099-01-01T00:00:00Z' }, maxCostUsd: 1, missed: 'skip', inputs: { name: 'x', count: 2, enabled: true } });",
    '// @ts-expect-error schedule inputs are scalars, unlike run inputs',
    "void client.schedule('flow.nika', { id: 'daily', when: { kind: 'once', at: '2099-01-01T00:00:00Z' }, maxCostUsd: 1, missed: 'skip', inputs: { nested: {} } });",
    '// @ts-expect-error null is not a schedule input scalar',
    "void client.schedule('flow.nika', { id: 'daily', when: { kind: 'once', at: '2099-01-01T00:00:00Z' }, maxCostUsd: 1, missed: 'skip', inputs: { empty: null } });",
    "void client.scheduleStatus('daily').then((status) => { const inputs: Record<string, string> | undefined = status.definition.inputs; void inputs; });",
    ...
    "const remoteLimits: NikaCompileRemoteLimits = { repairs: 0, maxTokens: 2048, callTimeoutMs: 5000, deadlineMs: 10000 };",
    "const remoteAuthoring: NikaCompileRemoteAuthoring = { cognition: 'explicitProvider', limits: remoteLimits };",
    "void client.compile('intent', { remoteAuthoring });",
    "if (compileOutcome.replayToken) void client.compile({ intent: 'intent', answers: { model: 'runtime/choice' } }, { remoteAuthoring: { cognition: 'deterministicOnly', replayToken: compileOutcome.replayToken } });",
    '// @ts-expect-error remote authoring does not select the operator model',
    "const remoteModel: NikaCompileRemoteAuthoring = { cognition: 'explicitProvider', model: 'provider/model' };",
    '// @ts-expect-error deterministic replay requires a token',
    "const tokenless: NikaCompileRemoteAuthoring = { cognition: 'deterministicOnly' };",
    '// @ts-expect-error replay cannot purchase a fresh authoring budget',
    "const replayLimits: NikaCompileRemoteAuthoring = { cognition: 'deterministicOnly', replayToken: 'token', limits: remoteLimits };",
    'void remoteModel; void tokenless; void replayLimits;',
